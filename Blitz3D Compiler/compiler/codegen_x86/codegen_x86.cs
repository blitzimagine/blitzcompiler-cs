...
namespace codegen_86
{
    public class Codegen_x86 : Codegen
    {
        ...
        public override void leave(TNode cleanup, int pop_sz)
        {
            ...
            foreach(string t in Tile.codeFrags)
            {
                ...
                else
                {
                    if (esp_off!=0)
                    {
                        @out.Write(fixEsp(esp_off));
                        esp_off = 0;
                    }
                    @out.Write(t);
                }
            }
            if (esp_off!=0) @out.Write(fixEsp(esp_off));
        ...
        {
            if (l.Length>0) Tile.dataFrags.Add(l + ":");
            Tile.dataFrags.Add("\t.dd\t" + i.ToString() + '\n');
        }
        ...
        {
            Tile.dataFrags.Add("\t.align\t" + n.ToString() + '\n');
        }
        ...
        private Tile genCompare(TNode t, out string func, bool negate)
        {
            switch (t.op)
            {
            case IR.SETEQ: func = negate ? "nz" : "z";
                break;
            case IR.SETNE: func = negate ? "z" : "nz";
                break;
            case IR.SETLT: func = negate ? "ge" : "l";
                break;
            case IR.SETGT: func = negate ? "le" : "g";
                break;
            case IR.SETLE: func = negate ? "g" : "le";
                break;
            case IR.SETGE: func = negate ? "l" : "ge";
                break;
            default:
                func = null;
                return null;
        ...
        private Tile munchLogical(TNode t)
        {
            ...
            switch (t.op)
            {
            case IR.AND: s = "\tand\t%l,%r\n";
                break;
            case IR.OR: s = "\tor\t%l,%r\n";
                break;
            case IR.XOR: s = "\txor\t%l,%r\n";
                break;
            default: return null;
            }
        ...
        private Tile munchArith(TNode t)
        {
            if (t.op == IR.DIV)
            {
                if (t.r.op == IR.CONST)
                {
                    ...
                    {
                        return new Tile("\tsar\t%l,byte " + shift.ToString() + "\n", munchReg(t.l));
                    }
                }
            ...
            if (t.op == IR.MUL)
            {
                if (t.r.op == IR.CONST)
                {
                    if (getShift(t.r.iconst, out int shift))
                    {
                        return new Tile("\tshl\t%l,byte " + shift.ToString() + "\n", munchReg(t.l));
                    }
                }
            ...
            {
                return new Tile(op + "%l," + s1 + "\n", munchReg(t.l));
            }
            if (t.op != IR.SUB && matchMEMCONST(t.l, out string s2))
            {
                return new Tile(op + "%l," + s2 + "\n", munchReg(t.r));
            }
        ...
        private Tile munchFPArith(TNode t)
        {
            ...
            switch (t.op)
            {
            case IR.FADD: s = "\tfaddp\tst(1)\n";
                break;
            case IR.FMUL: s = "\tfmulp\tst(1)\n";
                break;
            case IR.FSUB: s = "\tfsubrp\tst(1)\n";
                s2 = "\tfsubp\tst(1)\n";
                break;
            case IR.FDIV: s = "\tfdivrp\tst(1)\n";
                s2 = "\tfdivp\tst(1)\n";
                break;
            default: return null;
            }
        ...
        private Tile munchCall(TNode t)
        {
            ...
            if (t.l.op == IR.GLOBAL)
            {
                q = new Tile("\tcall\t" + t.l.sconst + "\n", t.r!=null ? munchReg(t.r) : null);
            } else
            {
                q = new Tile("\tcall\t%l\n", munchReg(t.l), t.r!=null ? munchReg(t.r) : null);
            }
            q.argFrame = t.iconst;
            q.want_l = Reg.EAX;
        ...
        private Tile munch(TNode t) //munch and discard result
        {
            ...
            case IR.MOVE:
                if (matchMEM(t.r, out string s))
                {
                    ...
                    if (q is null) q = new Tile("\tmov\t" + s + ",%l\n", munchReg(t.l));
                }
                break;
            }
        ...
        private Tile munchReg(TNode t) //munch and put result in a CPU reg
        {
            ...
            case IR.NEG:
                q = munchUnary(t);
                break;
            case IR.AND: case IR.OR: case IR.XOR:
                q = munchLogical(t);
                break;
            case IR.ADD: case IR.SUB: case IR.MUL: case IR.DIV:
                q = munchArith(t);
                break;
            case IR.SHL: case IR.SHR: case IR.SAR:
                q = munchShift(t);
                break;
            case IR.SETEQ: case IR.SETNE: case IR.SETLT: case IR.SETGT: case IR.SETLE: case IR.SETGE:
                q = munchRelop(t);
                break;
            case IR.FSETEQ: case IR.FSETNE: case IR.FSETLT: case IR.FSETGT: case IR.FSETLE: case IR.FSETGE:
                q = munchFPRelop(t);
                break;
            default:
                q = munchFP(t);
        ...
        private Tile munchFP(TNode t) //munch and put result on FP stack
        {
            ...
            switch (t.op)
            {
                ...
                case IR.FNEG:
                    q = munchFPUnary(t);
                    break;
                case IR.FADD: case IR.FSUB: case IR.FMUL: case IR.FDIV:
                    q = munchFPArith(t);
                    break;
                default:
                    q = munchReg(t);
        ...
        private static bool matchCONST(TNode t, out string s)
        {
            ...
            switch (t.op)
            {
            case IR.CONST: s = t.iconst.ToString();
                return true;
            case IR.GLOBAL: s = t.sconst;
                return true;
            }
            s = null;
            return false;
